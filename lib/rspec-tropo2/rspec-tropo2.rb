...
RSpec::Matchers.define :be_a_valid_ask_event do
  match_for_should do |event|
    ...
    unless event.namespace_href == 'urn:xmpp:ozone:ext:1'
      @error = "expected urn:xmpp:ozone:ext:1 for namespace_href - got #{event.namespace_href}"
      raise RSpec::Expectations::ExpectationNotMetError
    end

    unless event.reason.namespace_href == 'urn:xmpp:ozone:ask:complete:1'
...
RSpec::Matchers.define :be_a_valid_complete_hangup_event do
  match_for_should do |event|
    ...
    unless event.reason.namespace_href == 'urn:xmpp:ozone:ext:complete:1'
...
RSpec::Matchers.define :be_a_valid_conference_command do
  match_for_should do |event|
    ...
    unless event.namespace_href == 'urn:xmpp:ozone:conference:1'
...
RSpec::Matchers.define :be_a_valid_hangup_event do
  match_for_should do |event|
    ...
    unless event.namespace_href == 'urn:xmpp:ozone:1'
...
RSpec::Matchers.define :be_a_valid_ringing_event do
  match_for_should do |event|
    ...
    unless event.namespace_href == 'urn:xmpp:ozone:1'
      @error = "expected urn:xmpp:ozone:1 for xmlns - got #{event.namespace_href}"
      raise RSpec::Expectations::ExpectationNotMetError
    end
...
RSpec::Matchers.define :be_a_valid_say_event do
  match_for_should do |event|
    ...
    unless event.reason.namespace_href == 'urn:xmpp:ozone:say:complete:1'
...
RSpec::Matchers.define :be_a_valid_stopped_say_event do
  match_for_should do |event|
    ...
    unless event.reason.namespace_href == 'urn:xmpp:ozone:say:1'
...
RSpec::Matchers.define :be_a_valid_transfer_event do
  match_for_should do |event|
    ...
    unless event.reason.namespace_href == 'urn:xmpp:ozone:transfer:complete:1'
